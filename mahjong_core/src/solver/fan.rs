    ...
    let results: Vec<FanSolveResult> = best_paths
        ...
        .map(|bp| {
            let fans: Vec<FanInstance> = bp
                ...
                .collect();
            // Deduplicate by sig_key in case of redundant paths
            let mut seen = FanExclusionSet::default();
            let mut unique = Vec::new();
            for f in fans {
                let key = ((f.fan_type as u16 as u64) << 17)
                    | ((f.uses_pair as u64) << 16)
                    | (f.used_set_mask & 0xFFFF);
                if !seen.is_set(f.fan_type) {
                    // Actually we need a separate seen set for sig_key, not fan_type
                    // But for dedup, checking fan_type is sufficient for single-instance fans
                    seen.set(f.fan_type);
                    unique.push(f);
                }
            }
...
/// Run the full scoring pipeline: extract candidates, embed exclusion masks, search.
pub fn score_hand(
    decomp: &super::decomposition::Decomposition,
    ctx: &super::fan_context::FanContext,
) -> Vec<FanSolveResult> {
    let raw = super::rules::check_all(decomp, ctx);
...
mod tests {
    ...
    fn candidate(
        ...
        // Exclusion mask computed by registry in production; for tests, use static.
